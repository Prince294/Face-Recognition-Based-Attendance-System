...
def contact():
    mess._show(title='Contact us',
               message="Please contact us on : 'champprince92@gmail.com")

###################################################################################


def check_haarcascadefile():
    exists = os.path.isfile("MainFiles\haarcascade_frontalface_default.xml")
    if exists:
        pass
    else:
...
def TakeImages():
    ...
    assure_path_exists("faceRecognizing/")
    serial = 0
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        harcascadePath = "MainFiles\haarcascade_frontalface_default.xml"
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.imwrite("TrainingImage\ " + name + "." + str(
                    serial) + "." + Id + '.' + str(sampleNum) + ".jpg", gray[y:y + h, x:x + w])

                image = cv2.imread("TrainingImage\ " + name + "." +
                                   str(serial) + "." + Id + '.' + str(sampleNum) + ".jpg")

                imgResize = cv2.resize(
                    image, (220, 220), interpolation=cv2.INTER_AREA)

                os.remove("TrainingImage\ " + name + "." + str(serial) +
                          "." + Id + '.' + str(sampleNum) + ".jpg")

                adjusted = cv2.addWeighted(imgResize, 0.85, imgResize, 0, 1)

                cv2.imwrite("TrainingImage\ " + name + "." + str(serial) + "." +
                            Id + '.' + str(sampleNum) + ".jpg", adjusted)

                cv2.imshow('Taking Images', img)

            if cv2.waitKey(100) & (0xFF == ord('q') | 0xFF == ord('Q')):
                break
...
def TrackImages():
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            cv2.imwrite("faceRecognizing\ " + "." + str(j) + "." + ".jpg",
                        gray[y:y + h, x:x + w])
            image = cv2.imread("faceRecognizing\ " +
                               "." + str(j) + "." + ".jpg")

            imgResize = cv2.resize(image, (220, 220))

            os.remove("faceRecognizing\ " +
                      "." + str(j) + "." + ".jpg")

            adjusted = cv2.addWeighted(imgResize, 0.85, imgResize, 0, 1)

            # cv2.imwrite(r"D:\OpenCV\SIH Final\New folder\faceRecognizing\ " +
            #             "." + str(j) + "." + ".jpg", adjusted)

            imageGray = cv2.cvtColor(adjusted, cv2.COLOR_BGR2GRAY)

            j += 1
            serial, conf = recognizer.predict(imageGray)
            if (conf < 50):
                ts = time.time()
            ...
            cv2.putText(im, str(bb), (x, y + h), font, 1, (255, 255, 0), 2)
...
datef = tk.Label(frame4, text="    "+day+"-"+mont[month]+"-"+year+"  |  ", fg="orange",
                 bg="#262523", width=70, height=1, font=('times', 22, ' bold '))
